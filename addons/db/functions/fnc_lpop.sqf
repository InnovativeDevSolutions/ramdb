 ...
 *
 * Example:
 * ["myKey", 1] call ramdb_db_fnc_lpop; (Server or Singleplayer only)
 * ["myKey", 1, myObject, "myFunction"] remoteExec ["ramdb_db_fnc_lpop", 2, false]; (Multiplayer only)
 *
 * Public: Yes
 */
...
if (_key == "") exitWith {
    diag_log text format ["ArmaRAMDb: 'ramdb_db_fnc_lpop' Invalid Input for Key '%1'", _key];
};

DB_COMMON(_key,[ARR_5(QUOTE(lpop),[ARR_1(_count)],_obj,_fnc,_call)]);
